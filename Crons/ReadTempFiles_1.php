...
if($nRun == 0){
    ...
    foreach ($LTF as &$TF) {
        ...
        $Row = 0;
        $iTemAnt = "";
        //Clear Table if apply
        switch ($TF_TFile){
            case "ShortGroup":
                $cOK = 0;
                //Code
                $codex = explode("|", $TF_txtValue);
                $gGroupCode = $codex[0];
                $gGroupName = $codex[1];
                
                $sGroup = "SELECT * FROM AssGroup WHERE GroupCode LIKE '$gGroupCode' ORDER BY idGroup ASC LIMIT 0, 1";
            ...
            while (($TFData = fgetcsv($Gestor, 0, ",")) !== FALSE) {
                ...
                switch ($TF_TFile):
                    case "ShortGroup":
                        ...
                        if(!empty($C0)){
                            //Busca existe BOM
                            $sBOMA = "SELECT * FROM BOM WHERE Parent_Part_Assembly LIKE '$C0' AND Item_Nbr NOT LIKE '' ORDER BY Item_Nbr ASC LIMIT 0, 1";
        ...
        switch ($TF_TFile){
            case "ShortGroup":
                //Solo si es mayor
                if($cOK > 0){
                    $tableResult .= "<table width=\'400\' border=\'1\' cellpadding=\'0\' cellspacing=\'0\'>"
                        . "<tr>"
                        . "<td align=\'center\' bgcolor=\'#c7c7c7\'>Assy</td>"
                        . "<td align=\'center\' bgcolor=\'#c7c7c7\'>Result</td>"
                        . "</tr>";
                    //Elimina Assy
                    $dAssy = "DELETE FROM AssembliesGroup WHERE idGroup = '$gGroupCode'";
                    $cnx->query($dAssy);
                    $countr = 0;
                    foreach ($listAss as &$Ass) {
                        ...
                        if($Assr == "OK"){
                            //Inserta el ensamble
                            $iAssy = "INSERT INTO AssembliesGroup VALUES('0', '0', '$gGroupCode', '$gAssy', '$dtAhora', '$uIDUser', '$uFullName', 'Active')";
                            $rAssy = $cnx->query($iAssy);
                            $cfont = "yellowgreen";
                        } else {
                            $cfont = "red";
                        ...
                        $tableResult .= "<tr bgcolor=\'$colorb\'>"
                            . "<td align=\'left\'>$gAssy</td>"
                            . "<td align=\'left\'><font style=\'color:$cfont;\'>$Assr</font></td>"
                            . "</tr>";
                ...
                $ToEmail = "alejandro.ochoa@masterworkelectronics.com";
