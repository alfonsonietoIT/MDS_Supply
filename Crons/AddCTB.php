...
/***************************************************************************** Valida Customer ***********************/
$sCustomer = "SELECT Customer, Class_Code FROM PodData WHERE Customer != '' GROUP BY Customer, Class_Code ORDER BY Customer";
$sCustomer = "SELECT Customer, Class_Code FROM PodData WHERE Customer != '' AND Class_Code = 'APO' GROUP BY Customer, Class_Code ORDER BY Customer";
...
if($nCustomer > 0){
    ...
    foreach ($rCustomer AS &$rCx){
        ...
        $iPP = "INSERT INTO ConsultPP VALUES('0', '$RANDx', '$dtAhora', '$gClassCode', '$gCustomer', '0', '')";
        //echo $iPP . "\r\n";
        $rPP = $cnx->query($iPP);
        //Array Fridays
        $nweeks = 12;
        ...
        if($nAssy > 0){
            ...
            $sPart = "SELECT Item_Nbr, "
                        . "GROUP_CONCAT(DISTINCT CONCAT(Parent_Part_Assembly,':',Qty_Assy) ORDER BY Parent_Part_Assembly ASC SEPARATOR '>') AS Ass "
                        . "FROM BOM WHERE $ExtraBOM GROUP BY Item_Nbr ORDER BY Item_Nbr ASC";
            //echo $sPart;
            ...
            foreach ($rPart AS &$rP){
                ...
                */
                ...
                $iTemp = "INSERT INTO TempProductionPlan VALUES("
                        . "'0', '$Ass', '$NAss', '$RANDx', '$dtAhora', '$Item', '$gDescI', '$gMfgr', '$gMfgrNP', '$gOH', '$gOv', '$gCo', '$tExpired', '$LeadTime', '$EngStr', "
                        . "'$w1', '$wp1', '$wdp1', "
                        . "'$w2', '$wp2', '$wdp2', "
                        . "'$w3', '$wp3', '$wdp3', "
                        . "'$w4', '$wp4', '$wdp4', "
...
$SubjectEmail = "(MDS) Production Plan Ready ($dtahorita2)";
