...
/***************************************************************************** Valida Customer ***********************/
$sCustomer = "SELECT Customer, Class_Code FROM PodData WHERE Class_Code != '' GROUP BY Customer, Class_Code ORDER BY Customer";
$sCustomer = "SELECT Customer, Class_Code FROM PodData WHERE Class_Code = 'ZMO' GROUP BY Customer, Class_Code ORDER BY Customer";
...
if($nCustomer > 0){
    ...
    foreach ($rCustomer AS &$rCx){
        ...
        while ($cweeks < $nweeks){
            ...
            switch ($cweeks){
                case 12:
                    $nextFridayAnt = $nextFriday;
                    $nextFriday = date('Y-m-t', strtotime($nextFriday));
                    
                    if(strtotime($nextFridayAnt) == strtotime($nextFriday)){
                        $nextFriday = date('Y-m-t', strtotime('+4 days', strtotime($nextFriday)));
                    }
                ...
                case 15:
                case 16:
                    $nextFriday = date('Y-m-t', strtotime('+1 days', strtotime($nextFriday)));
                    $ArrFriday[] = $nextFriday;
                    break;
                default :
...
$ToEmail = "it.systems@masterworkelectronics.com";
//$ToEmail = "mario.chavez@masterworkelectronics.com";
$FromEmail = "scanner@masterworkelectronics.com";
