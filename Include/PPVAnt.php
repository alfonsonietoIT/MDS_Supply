    ...
    case "AddPPV":
        switch($sIV){
            case "sAdd":
                ...
                if(!empty($gClassCode) && $gAcctg > 0 && !empty($gPN) && $gQty > 0 && $gPrice > 0 && !empty($gCode) && strlen($gCode) == 6):
                    //Valida si existe
                    ...
                    if($nEx == 0):
                        ...
                        $sOPO = "SELECT * FROM PPVs WHERE Vendor_Number = '$gCode' AND Part_Nbr = '$gPN' AND "
                                . "Currency_List_Unit_Price = '$gPrice' AND Acctg_value = '$gAcctg' "
                                . "AND Class_Code = '$gClassCode' ORDER BY idPPVs DESC LIMIT 0, 1";
            ...
            case "Home":
                ...
                if($uAccount == "Buyer" || $uAccount == "SuperUser" || $uAccount == "Admin"){
                    ...
                    $aFields = array(
                        ...
                        array("", "Part Number", "partnumber", "text", "text", "partnumber", "", "", "", ""),
		...
		$linkfile = "TempFiles/" . $nombre_archivo; 
                    ...
                    if($extension == "csv"){
                        ...
                        if (move_uploaded_file($_FILES['ufile']['tmp_name'], $linkfile)){
                            ...
                            $siFile = "INSERT INTO TempFiles VALUES("
                                    . "'$ID', '$TFile', '$link', "
                                    . "'$uIDUser', '$uFullName', 'Open', '$dtAhora', '$dtAhora', '$dtAhora'"
                                    . ")";
                            $iFile = $cnx->query("$siFile");
            ...
            case "Home":
                ...
                if(isset($_GET['search'])){
                    ...
                    $LQry = "SELECT * FROM TempFiles WHERE Status = 'Open' OR Status = 'Updated' OR Status = 'Runing' ORDER BY dtCreated ASC";
                }
    ...
    case "Home":
        switch ($sIV){
            ...
            case "Closed":
                ...
                switch ($sIV){
                    case "Home":
                        ...
                        if(isset($_GET['search'])){
                            ...
                            $LQry = "SELECT * FROM PPVs WHERE Status != 'Closed' ORDER BY idPPV ASC";
                        }
                        break;
                    case "Closed":
                        //Titulo, Size, Info, Alert, Search, DataSearch
                ...
                $aColumns = array(
                    array("", "tr_class", "tit_col_center", "#", "tit_col_center", "+", "tit_col_center", "BCode", "tit_col_center", "PPV", "tit_col_center", "NParte", "tit_col_center", "$orderpo PO", "tit_col_center", "$orderimp Impact", "tit_col_center", "$orderst Status")
                );
                $LRows = $cnx->query("$LQry")->fetchAll();
                foreach ($LRows as &$LRowsx) {
                    $count++;
